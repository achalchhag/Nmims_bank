...
import com.nmimsbank.bank.Models.Transaction;
import com.nmimsbank.bank.Views.TransactionCellFactory;
...
public class DashboardController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        send_btn.setOnAction(e -> onSendMoney());
        accountsSummary();
    }
    ...
    private void bindData(){
        ...
        var chBalanceProp = Model.getInstance().getClient().checkingAccountProperty().get().balanceProperty();
        checking_bal.textProperty().bind(Bindings.createStringBinding(
                () -> String.format("$%.2f", chBalanceProp.get()),
                chBalanceProp
        ));
        checking_acc_num.textProperty().bind(Model.getInstance().getClient().checkingAccountProperty().get().accountNumberProperty());

        var svBalanceProp = Model.getInstance().getClient().savingsAccountProperty().get().balanceProperty();
        savings_bal.textProperty().bind(Bindings.createStringBinding(
                () -> String.format("$%.2f", svBalanceProp.get()),
                svBalanceProp
        ));
